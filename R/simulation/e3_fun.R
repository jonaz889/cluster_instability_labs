...
e3_func <- function(sigma){
  ...
  #dist <- seq(0, 2000, length.out=n_dist)
  side_length <- 20
  pull <- seq(0,5, length.out = n_dist)
  # Cluster parameters
  sx <- lapply(1:n_dist, function(x){c(sigma,sigma, sigma)}) # x scale
  sy <- lapply(1:n_dist, function(x){c(sigma,sigma, sigma)})     # y scale
  mx <- lapply(1:n_dist, function(x){c(side_length/2, -side_length/2, 0)}) # x shift
  my <- lapply(1:n_dist, function(x) {c(0, 0, (side_length * sqrt(3))/2 + pull[x])})  
  # Construct dataset
  X <- lapply(1:n_dist, function(x){
    cbind(
      x_r * sx[[x]][g] + mx[[x]][g],
      y_r * sy[[x]][g] + my[[x]][g]
    )
  ...
  kms <- lapply(1:n_dist, function(x){kmeans(X[[x]], centers = k, nstart = 20)})
  
  ari <- unlist(lapply(1:n_dist, function(t)
  ...
  return(list(
    ...
    kms = kms,
    clusters = lapply(1:n_dist, function(t) kms[[t]]$cluster),
    ari = ari
  ))
}
...
bin_clusterings_by_ari <- function(sigma, R = 50, efunc) {
  ...
  bins_per_pull <- vector("list", n_dist)
