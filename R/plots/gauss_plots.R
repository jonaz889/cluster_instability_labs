...
plot_kmeans <- function(X, pred_lab, true_lab, dist, mis_rate)
{
  plot(X, xlim=range(X[,1]), ylim=range(X[,2]),
       xlab="", ylab="",main = bquote(
         atop(
           d(mu[1], mu[2]) == .(round(dist, 2)),
           "mis_rate =" ~ .(round(mis_rate, 3))
         )
       ), axes=FALSE,
       col=pred_lab+1, pch=15+true_lab, asp=1)
  
  points(X[pred_lab != true_lab, , drop = FALSE], cex = 1.3, pch = 1)
  box()
}

# Assumes single sigma run list i.e. no reps.
...
plot_mis_dist_over_sigma <- function(run_sigma_list, xlim=NULL) {
  sigmas <- sapply(run_sigma_list, function(run) run$sigma)
  cols <- seq_along(run_sigma_list)
